...
var det=fails.map(function(f){return "<details><summary><span class='badge text-bg-danger'>FAIL</span> "+esc(f.title)+" <code>"+f.sc+"</code> <span class='badge text-bg-secondary'>"+f.level+"</span> <span class='badge text-bg-dark'>"+f.sev+"</span></summary><p class=mb-1><strong>What you need to do:</strong> "+esc(f.fix)+"</p>"+(f.ev.length?"<div class=evidence>"+f.ev.map(esc).join("\n")+"</div>":"")+"</details>";}).join("");
